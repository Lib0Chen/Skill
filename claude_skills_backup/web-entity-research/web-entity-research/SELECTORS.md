...
你可以先在 config.json 的 selectors.content_root_selectors 里填入某个 selector，然后跑一条样本，看 scraped_data.csv 的 raw_content 是否只包含目标区域。
