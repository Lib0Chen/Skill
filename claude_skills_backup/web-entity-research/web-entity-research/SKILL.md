...
- 工具：Playwright（本地 Chrome channel），scripts/url_discovery.py
- 输入：mapping.csv + config.json（site.base_url、site.search_url_template、selectors.result_selectors；site.search_mode 可选）。
...
- 输入：verified_mapping.csv + config.json（selectors.content_root_selectors 可选）。
...
### 5) 合并回全量清单（Left Join + Propagate，可选）
- 做什么：以 Target_URL 为 key 左连接回全量清单；同一 URL 的多个行共享同一抓取结果；对错配 URL 做人工修正后重跑 scraper/parser。
- 工具：pandas（建议在项目里单独写一个 merge 脚本，或在笔记本里做一次性处理）
...
1) config.json：site.* + selectors.* + parser.* + io.* + browser.*
2) mapping.csv（Search_Key, Search_Query）
3) 输出 schema（列名+规则），以及是否需要窗口/时点逻辑

脚本入口参考：
- scripts/url_discovery.py
- scripts/scraper.py
- scripts/parser.py
